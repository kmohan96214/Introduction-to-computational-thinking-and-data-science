...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        for i in range(numViruses):
            listi.append(SimpleVirus(maxBirthProb,clearProb))   
        pat = Patient(listi,maxPop)
        for j in range(300):
            pat.update()
            lee[j].append(pat.getTotalPop())
            
            
    x = [float(mean(lee[i])) for i in range(300)]
    #y = [i for i in range(300)]
    pylab.plot(x, label="Total Virus Population")
        ...
        for index, virus in reversed( list( enumerate( self.viruses ) ) ):
            if virus.doesClear():
                ...
                try:
                    ...
                    newViruses.append( virus.reproduce( popDensity ,self.prescription) )
                except NoChildException:
                    continue
